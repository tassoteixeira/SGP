...
Begin VB.Form consulta_baixa_nota 
   Caption         =   "Consulta Baixas de Notas de Abastecimento"
   ClientHeight    =   5355
   ClientLeft      =   1200
   ClientTop       =   1875
   ClientWidth     =   8655
   ...
   Picture         =   "con_baixa_nota.frx":0000
   ScaleHeight     =   5355
   ScaleWidth      =   8655
   ...
   Begin VB.CommandButton cmd_ok 
      ...
      Height          =   855
      Left            =   7020
      Picture         =   "con_baixa_nota.frx":0046
      Style           =   1  'Graphical
      TabIndex        =   7
   ...
   Begin VB.CommandButton cmd_sair 
      ...
      Height          =   855
      Left            =   7860
      Picture         =   "con_baixa_nota.frx":1650
      Style           =   1  'Graphical
      TabIndex        =   8
   ...
   End
End
Attribute VB_Name = "consulta_baixa_nota"
...
Private Sub AtualizaGrid()
    ...
    If ValidaCampos Then
        lSQL = ""
        lSQL = lSQL & "SELECT Baixa_Nota_Abastecimento.[Data do Abastecimento], Baixa_Nota_Abastecimento.Periodo, Baixa_Nota_Abastecimento.[Tipo do Movimento], Baixa_Nota_Abastecimento.[Numero da Nota], Baixa_Nota_Abastecimento.Ordem, Baixa_Nota_Abastecimento.[Codigo do Cliente], Cliente.[Razao Social], Produto.Nome, Baixa_Nota_Abastecimento.[Valor Pago], Baixa_Nota_Abastecimento.[Codigo do Produto2], Baixa_Nota_Abastecimento.[Data do Pagamento], Baixa_Nota_Abastecimento.Empresa"
        lSQL = lSQL & "  FROM Baixa_Nota_Abastecimento, Cliente, Produto"
        If cbo_campo.ListIndex <> -1 Then
            ...
            lSQL = lSQL & "   AND Produto.Codigo = Baixa_Nota_Abastecimento.[Codigo do Produto2]"
            lSQL = lSQL & "   AND Baixa_Nota_Abastecimento." & fConsultaPreparaCondicao(cbo_campo.Text, rsDados.Fields(cbo_campo.Text).Type, cbo_operador.Text, txt_condicao.Text)
        End If
        lSQL = lSQL & " ORDER BY [Data do Pagamento], Periodo, [Numero da Nota], Ordem, [Codigo do Produto2]"
...
Private Sub FormataGrid()
    ...
    MSFlexGrid1.Rows = 1
    i = 0
    MSFlexGrid1.TextMatrix(0, i) = "Data Abast."
    ...
    i = i + 1
    MSFlexGrid1.TextMatrix(0, i) = "Tipo"
    MSFlexGrid1.ColAlignment(i) = flexAlignCenterCenter
    MSFlexGrid1.ColWidth(i) = 400
    i = i + 1
    MSFlexGrid1.TextMatrix(0, i) = "Número do Documento"
    MSFlexGrid1.ColAlignment(i) = flexAlignLeftCenter
    MSFlexGrid1.ColWidth(i) = 900
    i = i + 1
    MSFlexGrid1.TextMatrix(0, i) = "Ordem"
    MSFlexGrid1.ColAlignment(i) = flexAlignLeftCenter
    MSFlexGrid1.ColWidth(i) = 450
    i = i + 1
    MSFlexGrid1.TextMatrix(0, i) = "Cli."
    ...
    MSFlexGrid1.ColWidth(i) = 600
    i = i + 1
    MSFlexGrid1.TextMatrix(0, i) = "Dt.Pgto"
    MSFlexGrid1.ColAlignment(i) = flexAlignCenterCenter
    MSFlexGrid1.ColWidth(i) = 1000
    i = i + 1
    MSFlexGrid1.TextMatrix(0, i) = "Emp."
    MSFlexGrid1.ColAlignment(i) = flexAlignRightCenter
    MSFlexGrid1.ColWidth(i) = 600
...
Private Sub MarcaCelulas()
    g_string = ""
    If MSFlexGrid1.TextMatrix(MSFlexGrid1.RowSel, 3) <> "" Then
        g_string = MSFlexGrid1.TextMatrix(MSFlexGrid1.RowSel, 5) & "|@|"              'Cod.Cliente
        g_string = g_string & MSFlexGrid1.TextMatrix(MSFlexGrid1.RowSel, 10) & "|@|"  'Data Pgto
        g_string = g_string & MSFlexGrid1.TextMatrix(MSFlexGrid1.RowSel, 9) & "|@|"   'Cod.Produto
        g_string = g_string & MSFlexGrid1.TextMatrix(MSFlexGrid1.RowSel, 3) & "|@|"   'Num.Nota
        g_string = g_string & MSFlexGrid1.TextMatrix(MSFlexGrid1.RowSel, 4) & "|@|"   'Ordem
        g_string = g_string & MSFlexGrid1.TextMatrix(MSFlexGrid1.RowSel, 0) & "|@|"   'Data Abast
        g_string = g_string & MSFlexGrid1.TextMatrix(MSFlexGrid1.RowSel, 11) & "|@|"  'Empresa
        g_string = g_string & MSFlexGrid1.TextMatrix(MSFlexGrid1.RowSel, 1) & "|@|"   'Periodo
    End If
End Sub
...
Private Sub PreencheCampos()
    lSQL = "SELECT * FROM Baixa_Nota_Abastecimento WHERE Empresa = 0"
    Set rsDados = Conectar.RsConexao(lSQL)
    cbo_campo.Clear
    For Each fld In rsDados.Fields
