...
Private Sub cmd_imprimir_Click()
    Mid(gStringImpTermica, 1, 1) = "1"
    Mid(gStringImpTermica, 13, 1) = "P"
    Call Relatorio
    cmd_sair.SetFocus
End Sub
...
Private Sub cmd_localizar_Click()
    ...
    xVariavel = gStringImpTermica
    gStringImpTermica = lPesquisa
    localizar.Show 1
    If gStringImpTermica <> "" Then
...
Private Sub Form_Activate()
    If l_status = 0 Then
        ...
        End If
        l_status = 1
        If Mid(gStringImpTermica, 1, 1) = "1" Then
            Mid(gStringImpTermica, 13, 1) = "P"
            Relatorio
            cmd_sair_Click
        End If
...
Private Sub Form_Load()
    ...
    lNomeEmpresa = "Empresa Teste"
    g_impressora_matricial = False
    g_tamanho_impressora = 132

    lTituloRelatorio = RetiraGString(2)
    gStringImpTermica = RetiraGString(1)
    If Mid(gStringImpTermica, Len(gStringImpTermica) - 3, 4) = ".LOG" Then
        AtualizaPagina
    Else
        If Mid(gStringImpTermica, 1, 1) = "0" Then
            Mid(gStringImpTermica, 13, 1) = "V"
            AtualizaPagina
        End If
    End If
End Sub
Private Sub AtualizaPagina()
    Dim i As Integer
    lNomeArquivo = Mid(gStringImpTermica, 2, Len(gStringImpTermica) - 1)
    If Mid(lNomeArquivo, Len(lNomeArquivo) - 3, 4) = ".LOG" Then
        'Open Mid(gStringImpTermica, 2, Len(gStringImpTermica) - 1) For Input As #1
...
Function EnviaEmail() As Boolean
    ...
    With MAPIMessages1
        ...
        .MsgSubject = "SGP Cerrado - " & lNomeEmpresa & ": " & lTituloRelatorio
        .MsgNoteText = "Segue em anexo o " & lTituloRelatorio & Chr(10) & "Da Empresa: " & lNomeEmpresa
...
'Print #1, Chr(27) & "0" ' Muda o passo p/ 8 LPP
        ...
        ElseIf Mid(dados, 1, 18) = "@@Printer.FontName" Then
            If g_impressora_matricial = True Then
                If UCase(lNomeEmpresa) Like "*POUSO ALTO*" Then
...
Private Sub Form_Unload(Cancel As Integer)
    ...
    If Mid(gStringImpTermica, Len(gStringImpTermica) - 3, 4) <> ".LOG" Then
        Mid(gStringImpTermica, 13, 1) = "V"
        gArqTxt.DeleteFile (gDrive & gDiretorioData & Mid(gStringImpTermica, 2, Len(gStringImpTermica) - 1))
        Mid(gStringImpTermica, 13, 1) = "P"
        gArqTxt.DeleteFile (gDrive & gDiretorioData & Mid(gStringImpTermica, 2, Len(gStringImpTermica) - 1))
    End If
