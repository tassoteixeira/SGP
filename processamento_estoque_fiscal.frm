...
Begin VB.Form processamento_estoque_fiscal 
   Caption         =   "Processamento de Estoque Fiscal"
   ClientHeight    =   5565
   ClientLeft      =   165
   ClientTop       =   285
   ClientWidth     =   7410
   Icon            =   "processamento_estoque_fiscal.frx":0000
   KeyPreview      =   -1  'True
   LinkTopic       =   "Form2"
   PaletteMode     =   1  'UseZOrder
   Picture         =   "processamento_estoque_fiscal.frx":030A
   ScaleHeight     =   5565
   ScaleWidth      =   7410
   ...
   Begin VB.CommandButton cmd_sair 
      ...
      Height          =   855
      Left            =   4020
      Picture         =   "processamento_estoque_fiscal.frx":0750
      Style           =   1  'Graphical
      TabIndex        =   16
   ...
   Begin VB.CommandButton cmd_ok 
      ...
      Height          =   855
      Left            =   1680
      Picture         =   "processamento_estoque_fiscal.frx":1DE2
      Style           =   1  'Graphical
      TabIndex        =   6
   ...
   End
End
Attribute VB_Name = "processamento_estoque_fiscal"
...
Private Function CalculaEstoque(ByVal pCalcEstoque As Boolean, ByVal pCalcSubEstoque As Boolean, ByVal pCodigoProduto As Long, ByVal pCodigoTipoSubEstoque As Integer, ByVal pQuantidade As Currency, ByVal pSoma As Boolean) As Boolean
    CalculaEstoque = False
    If pSoma Then
    ...
    If pCalcEstoque Then
        'If Estoque.AlterarQuantidade(g_empresa, pCodigoProduto, pQuantidade, pSoma) Then 'old 21/07/15
            'CalculaEstoque = True'old 21/07/15
        If Estoque.AlterarQuantidadeTemporaria(g_empresa, pCodigoProduto, pQuantidade, pSoma) Then 'new 21/07/15
            CalculaEstoque = True 'new 21/07/15
        Else
            CalculaEstoque = False
...
Private Sub Processamento()
    ...
    If chk_calcula_saida.Value = 1 Then
        ...
        'neste formulario nao utilizara esta função
...
On Error GoTo trata_erro
    ...
    If (MsgBox("Na empresa " & g_nome_empresa & Chr(10) & "Será calculado as entradas de " & msk_data_inicial & " até " & msk_data_final & " para todo estoque." & Chr(10) & Chr(10) & "Deseja realmente fazer este processamento?", vbYesNo + 256, "Calcula Entradas para o Estoque!")) = 6 Then
        ...
        xSQL = ""
        'xSQL = xSQL & "SELECT [Codigo do Produto], Quantidade, SubEstoque"
        ...
        xSQL = xSQL & "   AND [Tipo da Entrada] = " & preparaTexto("1")
        ...
        If rsEntradaProduto.RecordCount > 0 Then
            Do Until rsEntradaProduto.EOF
                ...
                If Cfop.Operacao <> "=" Then
                    ...
                    If CalculaEstoque(True, True, rsEntradaProduto("Codigo do Produto").Value, rsEntradaProduto("SubEstoque").Value, rsEntradaProduto("Quantidade").Value, xOperacaoSoma) Then
                    Else
...
On Error GoTo trata_erro
    ...
    If (MsgBox("Na empresa " & g_nome_empresa & Chr(10) & "Será calculado as NFe de " & msk_data_inicial.Text & " até " & msk_data_final.Text & " para todo estoque." & Chr(10) & Chr(10) & "Deseja realmente fazer este processamento?", vbYesNo + 256, "Calcula NFe para o Estoque!")) = vbYes Then
        ...
        Conectar.IniciaTransacao
        xSQL = ""
        'xSQL = xSQL & "SELECT [Codigo de Produto], Quantidade, SubEstoque, CFOP"
        ...
        Else
            ...
            End If
        End If
        'xSQL = xSQL & " ORDER BY [Codigo de Produto], Data, CFOP" ..old 21/07/2015
        ...
        If rsNFeItem.RecordCount > 0 Then
            Do Until rsNFeItem.EOF
                ...
                If Cfop.Operacao <> "=" Then
                    ...
                    If CalculaEstoque(True, True, rsNFeItem("Codigo de Produto").Value, 1, rsNFeItem("Quantidade").Value, xOperacaoSoma) Then
                    Else
                        xAtualizou = False
                        Conectar.CancelaTransacao
...
On Error GoTo trata_erro
    ...
    If (MsgBox("Na empresa " & g_nome_empresa & Chr(10) & "Será calculado as saidas de cupom de " & msk_data_inicial.Text & " até " & msk_data_final.Text & " para todo estoque." & Chr(10) & Chr(10) & "Deseja realmente fazer este processamento?", vbYesNo + 256, "Calcula Saidas de Cupom para o Estoque!")) = 6 Then
        ...
        Conectar.IniciaTransacao
        xSQL = ""
        'xSQL = xSQL & "SELECT [Codigo do Produto], Quantidade, [Tipo do SubEstoque]"
        ...
        End If
        'xSQL = xSQL & " ORDER BY [Codigo do Produto], Data"  ..old 21/07/2015
        ...
        If rsMovimentoCupomFiscal.RecordCount > 0 Then
            Do Until rsMovimentoCupomFiscal.EOF
                If CalculaEstoque(True, True, rsMovimentoCupomFiscal("Codigo do Produto").Value, 1, rsMovimentoCupomFiscal("Quantidade").Value, xOperacaoSoma) Then
                Else
                    xAtualizou = False
                    Conectar.CancelaTransacao
...
'neste formulario nao utilizara esta função new 16/07
'daqui...
...
'        xSQL = xSQL & "   AND [Cupom Cancelado] = " & preparaBooleano(False)
'        xSQL = xSQL & "   AND [Item Cancelado] = " & preparaBooleano(False)
'        xSQL = xSQL & " ORDER BY [Codigo do Produto], Data"
...
'                If CalculaEstoque(True, True, rsMovimentoVendaConveniencia("Codigo do Produto").Value, 1, rsMovimentoVendaConveniencia("Quantidade").Value, xOperacaoSoma) Then
'                Else
''aquiaqui                    xAtualizou = False
''                    Conectar.CancelaTransacao
...
'neste formulario nao utilizara esta funcao new 16/07
'daqui ...
...
'        Conectar.IniciaTransacao
'        xSQL = ""
'        xSQL = xSQL & "SELECT [Codigo do Produto2], Quantidade, [Codigo do Tipo do SubEstoque]"
...
'            xSQL = xSQL & "   )"
'        End If
'        xSQL = xSQL & " ORDER BY [Codigo do Produto2], Data"
'
'
...
'                If CalculaEstoque(True, True, rsMovimentoLubrificante("Codigo do Produto2").Value, 1, rsMovimentoLubrificante("Quantidade").Value, xOperacaoSoma) Then
'                    If Me.chkTransferenciaSubEstoque.Value = 1 Then
'                        'Diminui no Sub-Estoque de Saída
'                        If CalculaEstoque(False, True, rsMovimentoLubrificante("Codigo do Produto2").Value, rsMovimentoLubrificante("Codigo do Tipo do SubEstoque").Value, rsMovimentoLubrificante("Quantidade").Value, xOperacaoSoma) Then
'                        Else
'                            xAtualizou = False
'                            Conectar.CancelaTransacao
...
On Error GoTo trata_erro
    ...
    If (MsgBox("Na empresa " & g_nome_empresa & Chr(10) & "Será somado as entradas (inventário do estoque inicial do dia) da data " & msk_data_inicial.Text & " no o estoque." & Chr(10) & Chr(10) & "Deseja realmente fazer este processamento?", vbYesNo + 256, "Soma Entradas do Inventário no Estoque!")) = vbYes Then
        ...
        Conectar.IniciaTransacao
        xSQL = ""
        xSQL = xSQL & "SELECT [Codigo do Produto], SubEstoque, SUM(Quantidade) AS Quantidade"
        xSQL = xSQL & "  FROM Entrada_Produto"
        xSQL = xSQL & " WHERE Empresa = " & g_empresa
        xSQL = xSQL & "   AND [Data da Entrada] >= " & preparaData(CDate(msk_data_inicial.Text))
        xSQL = xSQL & "   AND [Data da Entrada] <= " & preparaData(CDate(msk_data_final.Text))
        ...
        Else
            ...
            End If
        End If
        xSQL = xSQL & " GROUP BY [Codigo do Produto], SubEstoque"
        xSQL = xSQL & " ORDER BY [Codigo do Produto], SubEstoque"
        Set rsEntradaProduto = Conectar.RsConexao(xSQL)
        If rsEntradaProduto.RecordCount > 0 Then
            Do Until rsEntradaProduto.EOF
                If CalculaEstoque(True, True, rsEntradaProduto("Codigo do Produto").Value, rsEntradaProduto("SubEstoque").Value, rsEntradaProduto("Quantidade").Value, True) Then
                Else
'                    xAtualizou = False
'                    Conectar.CancelaTransacao
    ...
    If (MsgBox("Na empresa " & g_nome_empresa & Chr(10) & "Será movido o inventário Contábil da data " & msk_data_inicial & " para todo estoque." & Chr(10) & Chr(10) & "Deseja realmente fazer este processamento?", vbYesNo + 256, "Move Entradas para o Estoque!")) = vbYes Then
        ...
        If rsEstoque2.RecordCount > 0 Then
            Do Until rsEstoque2.EOF
                If CalculaEstoque(True, True, rsEstoque2("Codigo do Produto2").Value, 1, rsEstoque2("Quantidade").Value, True) Then
                Else
'                    xAtualizou = False
'                    Conectar.CancelaTransacao
        ...
        If rsTransfInternaEstoque.RecordCount > 0 Then
            Do Until rsTransfInternaEstoque.EOF
                ...
                If chkCalculoInverso.Value = 1 Then
                    xOperacaoSoma = True
                End If
                If CalculaEstoque(False, True, rsTransfInternaEstoque("Codigo do Produto").Value, rsTransfInternaEstoque("Codigo do SubEstoque de Saida").Value, rsTransfInternaEstoque("Quantidade").Value, xOperacaoSoma) Then
                Else
                    xAtualizou = False
                    Conectar.CancelaTransacao
...
'                End If
                ...
                If chkCalculoInverso.Value = 1 Then
                    xOperacaoSoma = False
                End If
                If CalculaEstoque(False, True, rsTransfInternaEstoque("Codigo do Produto").Value, rsTransfInternaEstoque("Codigo do SubEstoque de Entrada").Value, rsTransfInternaEstoque("Quantidade").Value, xOperacaoSoma) Then
                Else
                    xAtualizou = False
                    Conectar.CancelaTransacao
...
On Error GoTo trata_erro
    ...
    If (MsgBox("Na empresa " & g_nome_empresa & Chr(10) & "Será feito o processamento para zerar todo seu estoque." & Chr(10) & Chr(10) & "Deseja realmente fazer este processamento?", vbYesNo + 256, "Zeramento de Estoque!")) = 6 Then
        ...
        xSQL = xSQL & "   SET [Quantidade Temporaria] = 0"
        xSQL = xSQL & " WHERE Empresa = " & g_empresa
        If cboGrupo.ItemData(cboGrupo.ListIndex) > 0 Then
            xSQL = xSQL & "    AND [Grupo do Produto] = " & cboGrupo.ItemData(cboGrupo.ListIndex)
        End If
...
Function ValidaCampos() As Boolean
    ...
    ElseIf Not IsDate(msk_data_final.Text) >= IsDate(msk_data_inicial.Text) Then
        ...
        msk_data_final.SetFocus
    ElseIf chk_zera_estoque.Value = 0 And chkSomaEntradaInventario.Value = 0 And chkMoveInventarioContabil.Value = 0 And chk_calcula_entrada.Value = 0 And chk_calcula_saida.Value = 0 Then 'And Me.chkTransferenciaSubEstoque.Value = 0 Then
