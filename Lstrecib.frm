...
Begin VB.Form frm_emissao_recibo_formulario 
   Caption         =   "Emissão dos Recibos (Formulário)"
   ClientHeight    =   3045
   ClientLeft      =   2250
   ClientTop       =   2730
   ClientWidth     =   3750
   Icon            =   "Lstrecib.frx":0000
   LinkTopic       =   "Form2"
   LockControls    =   -1  'True
   PaletteMode     =   1  'UseZOrder
   Picture         =   "Lstrecib.frx":030A
   ScaleHeight     =   3045
   ScaleWidth      =   3750
   ...
   Begin VB.CommandButton cmd_imprimir 
      ...
      Height          =   855
      Left            =   720
      Picture         =   "Lstrecib.frx":19E2
      Style           =   1  'Graphical
      TabIndex        =   9
      ToolTipText     =   "Imprime o Recibo de Cheque (Formulário)."
      Top             =   2040
      Width           =   795
   End
   ...
   End
End
Attribute VB_Name = "frm_emissao_recibo_formulario"
...
Dim rsChequeFormulario As New adodb.Recordset
...
Private Sub ImprimeDados()
    ...
    Printer.FontName = "Arial"
    Printer.FontSize = 10
    Do Until rsChequeFormulario.EOF
'        If l_linha = 2 Then
...
'        End If
        ...
        Printer.Print "R$ " & Format(rsChequeFormulario!valor, "###,##0.00")
        Printer.FontSize = 10
        Printer.DrawWidth = 4
        ...
        x_extenso = FazExtenso(rsChequeFormulario!valor)
        ...
        Printer.CurrentY = l_linha_tab + 5
        Printer.Print rsChequeFormulario!Historico
        x_data = msk_data
        x2_data = msk_data
        ...
        x_tamanho_string = Printer.TextWidth(rsChequeFormulario![Nome do Favorecido])
        Printer.CurrentX = l_coluna_tab + ((20 - x_tamanho_string) / 2)
        Printer.CurrentY = l_linha_tab + 9
        Printer.Print rsChequeFormulario![Nome do Favorecido]
'
        ImprimeGrade l_linha
        Printer.NewPage
        l_linha = 0
'
        rsChequeFormulario.MoveNext
    Loop
'    ImprimeGrade l_linha
    Printer.EndDoc
End Sub
...
Private Sub Relatorio()
    ZeraVariaveis
'    tbl_cheque_formulario.Index = "id_numero"
'    tbl_cheque_formulario.Seek ">", g_empresa, (txt_numero_i - 1)
...
'    End If
    lSQL = ""
    lSQL = lSQL & " SELECT Valor, Historico, [Nome do Favorecido]"
    lSQL = lSQL & "   FROM Cheque_Formulario"
    lSQL = lSQL & "  WHERE Empresa = " & g_empresa
    lSQL = lSQL & "    AND [Numero do Cheque] >= " & preparaTexto(txt_numero_i.Text)
    lSQL = lSQL & "    AND [Numero do Cheque] <= " & preparaTexto(txt_numero_f.Text)
    
    Set rsChequeFormulario = Conectar.RsConexao(lSQL)
    If rsChequeFormulario.RecordCount > 0 Then
        If chk_formulario Then
            ImprimeDadosFormulario
        Else
            ImprimeDados
        End If
    End If
    rsChequeFormulario.Close
    Set rsChequeFormulario = Nothing
    cmd_sair.SetFocus
End Sub
Private Sub ImprimeDadosFormulario()
    ...
    Do Until rsChequeFormulario.EOF
        ...
        Printer.FontSize = 14
        Printer.DrawWidth = 6
        ImprimeCentralizado Format(rsChequeFormulario!valor, "###,##0.00"), 12.1, 18.1, 2.8, 1
        ...
        Printer.FontSize = 10
        Printer.DrawWidth = 4
        ImprimeTexto FazExtenso(rsChequeFormulario!valor), 4, 18, 5.2, 1
        
        ImprimeTexto rsChequeFormulario!Historico, 4, 18, 7.4, 1
        
        ImprimeCentralizado Trim(g_cidade_empresa) & ", " & Day(msk_data) & " de " & Format(msk_data, "mmmm") & " de " & Format(msk_data, "yyyy") & ".", 10, 18, 11.4, 1
        
        ImprimeCentralizado rsChequeFormulario![Nome do Favorecido], 10, 18, 12.8, 1
        rsChequeFormulario.MoveNext
    Loop
    Printer.EndDoc
End Sub
...
Function ValidaCampos() As Integer
    ...
    ElseIf Not Val(txt_numero_i) > 0 Then
        MsgBox "Informe o número inicial.", vbInformation, "Atenção!"
        txt_numero_i.SetFocus
    ElseIf Not Val(txt_numero_f) > 0 Then
        MsgBox "Informe o número final.", vbInformation, "Atenção!"
        txt_numero_f.SetFocus
    ElseIf txt_numero_f < txt_numero_i Then
        MsgBox "Número final deve ser maior que o número inicial.", vbInformation, "Atenção!"
        txt_numero_f.SetFocus
    Else
        ValidaCampos = True
    End If
