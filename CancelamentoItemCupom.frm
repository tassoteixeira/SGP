...
Begin VB.Form CancelamentoItemCupom 
   Caption         =   "Cancelamento de Ítem do Cupom Fiscal"
   ClientHeight    =   7590
   ClientLeft      =   1410
   ClientTop       =   1545
   ClientWidth     =   10950
   Icon            =   "CancelamentoItemCupom.frx":0000
   KeyPreview      =   -1  'True
   LinkTopic       =   "Form2"
   PaletteMode     =   1  'UseZOrder
   Picture         =   "CancelamentoItemCupom.frx":030A
   ScaleHeight     =   7590
   ScaleWidth      =   10950
   Begin VB.Frame frmDados
      ...
      Begin VB.CommandButton cmd_ok 
         ...
         Height          =   855
         Left            =   9060
         Picture         =   "CancelamentoItemCupom.frx":0750
         Style           =   1  'Graphical
         TabIndex        =   10
      ...
      Begin VB.CommandButton cmd_sair 
         ...
         Height          =   855
         Left            =   9960
         Picture         =   "CancelamentoItemCupom.frx":1D5A
         Style           =   1  'Graphical
         TabIndex        =   3
   ...
   End
End
Attribute VB_Name = "CancelamentoItemCupom"
...
Private rsCupomFiscal As New adodb.Recordset
...
Private Sub AtualizaGrid()
    ...
    xSQL = ""
    xSQL = xSQL & "   SELECT Movimento_Cupom_Fiscal.Ordem, Movimento_Cupom_Fiscal.[Codigo do Produto], "
    xSQL = xSQL & "          Movimento_Cupom_Fiscal.[Valor Unitario], Movimento_Cupom_Fiscal.Quantidade, "
    xSQL = xSQL & "          Movimento_Cupom_Fiscal.[Valor Total], Movimento_Cupom_Fiscal.[Item Cancelado], "
    xSQL = xSQL & "          Movimento_Cupom_Fiscal.[Cupom Cancelado], Produto.Nome AS NomeProduto"
    ...
    xSQL = xSQL & "      AND Movimento_Cupom_Fiscal.[Numero do Cupom] = " & lNumero
    ...
    Set rsCupomFiscal = New adodb.Recordset
    Set rsCupomFiscal = Conectar.RsConexao(xSQL)
    If Not rsCupomFiscal.EOF Then
        Do Until rsCupomFiscal.EOF
            i = i + 1
            fgd_composicao_caixa.Rows = fgd_composicao_caixa.Rows + 1
            fgd_composicao_caixa.Row = i
            fgd_composicao_caixa.Col = 0
            fgd_composicao_caixa.Text = Format(rsCupomFiscal("Ordem").Value, "###,##0")
            fgd_composicao_caixa.Col = 1
            fgd_composicao_caixa.Text = Format(rsCupomFiscal("Codigo do Produto").Value, "###,##0")
            fgd_composicao_caixa.Col = 2
            fgd_composicao_caixa.Text = rsCupomFiscal("NomeProduto").Value
            fgd_composicao_caixa.Col = 3
            fgd_composicao_caixa.Text = Format(rsCupomFiscal("Valor Unitario").Value, "###,###,##0.00")
            fgd_composicao_caixa.Col = 4
            fgd_composicao_caixa.Text = Format(rsCupomFiscal("Quantidade").Value, "###,###,##0.00")
            fgd_composicao_caixa.Col = 5
            fgd_composicao_caixa.Text = Format(rsCupomFiscal("Valor Total").Value, "###,###,##0.00")
            fgd_composicao_caixa.Col = 6
            If rsCupomFiscal("Item Cancelado").Value Or rsCupomFiscal("Cupom Cancelado").Value Then
                lCancelado(i) = True
                fgd_composicao_caixa.Text = "Cancelado"
            Else
                lCancelado(i) = False
                fgd_composicao_caixa.Text = "Normal"
                xTotal = xTotal + rsCupomFiscal("Valor Total").Value
            End If
            rsCupomFiscal.MoveNext
        Loop
    End If
    
    
    rsCupomFiscal.Close
    Set rsCupomFiscal = Nothing
...
Private Sub Form_Load()
    CentraForm Me
    lCodigoEcf = CDate(RetiraGString(1))
    lData = CDate(RetiraGString(2))
    lNumero = CLng(RetiraGString(3))
    g_string = ""
    lItemMarcado = 0
End Sub
